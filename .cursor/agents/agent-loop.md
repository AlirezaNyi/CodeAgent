...
description: >-
  ...
  durable approval resume, memory write/read in the loop, optional plan.nodes
  DAG waves, or Phase 3 subagent scheduler/model-router work.
model: inherit
---
...
- `crates/raya-agent/src/orchestrator.rs`, `resources.rs`, `lib.rs`, `subagent/` (including `dag.rs`)
- `crates/raya-agent/tests/e2e_mock_flow.rs`, `e2e_subagent.rs`, `e2e_approval_resume.rs`, `e2e_memory.rs`, `e2e_dag.rs`
- `crates/raya-core/src/models/task.rs` (`TaskPhase`, `AgentTask`, `TaskCheckpoint`)
- `crates/raya-core/src/models/plan.rs` usage for `DagNode` / `validate_dag` in the loop
...
- Ensure e2e still asserts phase `Completed` and key event kinds when relevant (`e2e_approval_resume`, `e2e_memory`, `e2e_dag`).

## Escalate when

- Changing HTTP approve/resume/dag API or CLI semantics (main + protocol)
