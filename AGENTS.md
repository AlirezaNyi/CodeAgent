...
| `persistence` | Store, migrations, index + memory/checkpoint tables | Schema intent | Migration path + Store API delta | Yes with safety; serialize with context on index schema | — |
| `agent-loop` | Orchestrator, TaskPhase, events, approvals, planio, memory write/read in loop | Store API / tool contracts | Loop code + e2e | Yes if files disjoint | Store API when schema changes |
